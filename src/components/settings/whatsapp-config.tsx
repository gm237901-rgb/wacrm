...
export function WhatsAppConfig() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (data.registered === false && data.registration_error) {
        toast.error(
          t('toastSavedButRegisterFailed', { error: data.registration_error }),
          { duration: 12000 },
        );
      ...
      if (payload.connected) {
        ...
        toast.success(
          payload.phone_info?.verified_name
            ? t('toastConnectedToName', { name: payload.phone_info.verified_name })
            : t('toastApiSuccess')
        );
      } else {
        setConnectionStatus('disconnected');
  ...
  async function handleVerifyRegistration() {
    ...
    try {
      ...
      if (data.live) {
        toast.success(t('toastVerifiedLive'));
      } else {
        toast.error(
